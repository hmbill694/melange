...
src/modules/project/message.rs — `ProjectMessage` enum with `SearchChanged(String)`, `ProjectsLoaded(Vec<Project>)`, and `LoadFailed(String)` variants; composed into root `Message::Project` in `app.rs`
...
src/ui/project.rs — `home_screen(projects, search_query, window_width)` pure view function; filters by name/description; renders 3-column card grid when width ≥ 900px, single-column list otherwise; each card shows name, optional description, and file path
...
src/ui/app/state.rs — Application UI state: `HomeScreenState` struct with `projects: Vec<Project>` and `search_query: String`, plus `Default` impl
src/ui/app/view.rs — `view_app()` function composing the main application view; implements 4-priority screen chain (opencode-not-found → db-error → loading → home screen)
